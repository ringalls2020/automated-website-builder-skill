...
Find local-business prospects and convert them into outreach-ready assets: qualified lead sheets, deduplicated campaign memory, a concise campaign brief from the user, Tocayo demo sites, Tocayo transfer links, 30-second walkthrough MP4s, and cold email/DM copy.
...
- Read `references/memory-workflow.md` before prospecting, lead selection, or site generation.
...
1. Load campaign memory.
   - Use `memory.md` in the active workspace or campaign output directory unless the user provides a different memory path.
   - If the file exists, read it before prospecting, selecting leads, building sites, or generating outreach.
   - If it does not exist, create it when the first finding or result is ready to record.
   - Follow `references/memory-workflow.md` for duplicate detection and append format.

2. Determine the entry path.
...
4. Load and normalize the lead sheet.
...
6. Interview the user for the campaign brief and outreach tone.
...
10. Update `memory.md`.
   - Append all selected, skipped, blocked, partial, and ready leads before final response.
   - Include enough normalized identifiers to prevent future duplicate prospecting or duplicate site generation.
   - Record live Tocayo URL, transfer link, MP4 path, outreach status, and date when available.
...
- Do not generate a new site for a remembered business unless the user explicitly asks to revisit it.
